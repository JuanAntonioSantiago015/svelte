    ...
    var Pool = class extends EventEmitter {
      ...
      _doCreateConnection(resolve, reject, timeoutEnd) {
        this._createConnection(timeoutEnd).then((conn) => {
          ...
          conn.once("error", function() {
            ...
            pool._endLeak(conn);
            delete __privateGet(pool, _activeConnections)[conn.threadId];
            while (currConn = __privateGet(pool, _idleConnections).peekAt(idx)) {
              if (currConn === conn) {
                __privateGet(pool, _idleConnections).removeOne(idx);
                continue;
              }
      ...
      async _createConnection(timeoutEnd) {
        ...
        conn.release = function(resolve) {
          if (__privateGet(pool, _closed) || !conn.isValid()) {
            pool._destroy(conn);
            resolve();
            return;
          }
